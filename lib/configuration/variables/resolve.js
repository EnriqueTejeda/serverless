...
class VariablesResolver {
  constructor({ servicePath, configuration, variablesMeta, sources, options }) {
    ...
    return Promise.all(Array.from(variablesMeta.keys()).map(this.resolveProperty));
  }
  async resolveVariables(propertyPath, valueMeta) {
    ...
    await Promise.all(
      valueMeta.variables.map(async (variableMeta) => {
        ...
        await this.resolveVariable(propertyPath, variableMeta);
  ...
  async resolveVariable(propertyPath, variableMeta) {
    ...
    do {
      ...
      let resolvedValue;
      try {
        resolvedValue = await this.resolveVariableSource(propertyPath, sourceData);
      } catch (error) {
  ...
  async resolveVariableSource(propertyPath, sourceData) {
    ...
    if (sourceData.params) {
      ...
      await Promise.all(
        sourceData.params.map(async (param) => {
          ...
          await this.resolveVariables(propertyPath, param);
    ...
    if (sourceData.address && sourceData.address.variables) {
      ...
      await this.resolveVariables(propertyPath, sourceData.address);
    ...
    const resultValue = await (async () => {
      try {
        return await this.resolveSource(propertyPath, sourceData);
      } catch (error) {
        if (isError(error)) {
  ...
  async resolveDependentProperty(dependentPropertyPath, dependencyPropertyPathKeys) {
    ...
    for (const [index, key] of dependencyPropertyPathKeys.entries()) {
      ...
      if (this.variablesMeta.has(depPropertyPath)) {
        ...
        await this.resolveProperty(depPropertyPath);
      }
    ...
    await Promise.all(
      Array.from(this.variablesMeta.keys()).map(async (propertyPath) => {
        ...
        await this.resolveProperty(propertyPath);
  ...
  memoizeMethods({
    resolveProperty: d(
      async function (propertyPath) {
        ...
        await this.resolveVariables(propertyPath, valueMeta);
        ...
        await Promise.all(Array.from(propertyVariablesMeta.keys()).map(this.resolveProperty));
      } /* We're fine with caching rejections here, hence no "promise: true" option */
    ),
    resolveSource: d(
      async function (propertyPath, sourceData) {
      ...
      {
        normalizer: ([, { type, params, address }]) => {
          return [
            type,
